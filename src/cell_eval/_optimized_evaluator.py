...
class OptimizedMetricsEvaluator:
    ...
    def _load_or_build_de_optimized(
        self, mode: str, de_path: pl.DataFrame | str | None,
        anndata_pair: PerturbationAnndataPair, de_method: str,
        num_threads: int, batch_size: int, outdir: str | None,
        prefix: str | None, pdex_kwargs: dict[str, Any] | None
    ) -> pl.DataFrame:
        """Load existing DE or compute new one"""
        ...
        return self._compute_de_robust(
            mode, adata, anndata_pair.control_pert,
            anndata_pair.pert_col, de_method, num_threads,
            batch_size, outdir, prefix, pdex_kwargs
        )

    def compute_metrics(self, metrics: list[str] | None = None) -> dict[str, Any]:
        """Compute evaluation metrics with optimization"""
        
        start_time = time.time()
        
        # Use MetricPipeline for computation
        ...
        results = pipeline.compute(metrics=metrics)
